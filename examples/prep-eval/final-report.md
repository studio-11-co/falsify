...
Offline verification (`run.sh`): tier L2. Registry-checked ordering, by hand: anchor(v1) 14:57:34.8 < anchor(v2) 14:57:40.2 < anchor(start) 14:57:46.4 < runner-reported `finished_at` 14:57:54.2; the start anchor is 476 ms after the runner-reported `started_at`. We do not call this tier L3 (see README, "The public receipts", for why).

What the verification shows: the bar in v2 existed no later than 14:57:40 UTC and has not changed; a start record naming that bar was on independent clocks before the runner says the run finished; the reported number clears the bar. What it does not show: that the number is correct, or that no earlier, unrecorded run took place. `started_at` and `finished_at` are the runner's own attestations; the anchors are not.
